  ...
  return (
    <div className="p-6 md:p-10 pb-32">
      <div className="flex justify-between items-center mb-6">
        <div>
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 leading-none">Pillar 2 — Distribution</p>
          ...
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            <thead className="bg-muted/40 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-bold text-muted-foreground uppercase tracking-wider text-center">Quality</th>
                ...
                <th className="px-6 py-4 font-bold text-muted-foreground uppercase tracking-wider text-right">Rev / Opp</th>
              ...
              {channels.map(c => (
                <tr key={c.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest ${c.engagementQuality === 'High' ? 'bg-emerald-100/50 text-emerald-700' : c.engagementQuality === 'Low' ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-600'}`}>{c.engagementQuality}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right font-bold text-emerald-600 text-[14px]">${c.revenue.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
