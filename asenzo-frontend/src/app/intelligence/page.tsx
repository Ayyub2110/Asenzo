...
export default function BusinessPulsePage() {
  ...
  const pulseMetrics = [
    { label: "Revenue", value: `$${pulse.revenue.toLocaleString()}` },
    { label: "Pipeline", value: `$${pulse.pipeline.toLocaleString()}` },
    { label: "Qualified Leads", value: pulse.qualifiedLeads },
    { label: "Conversion Rate", value: `${pulse.conversionRate}%` },
    { label: "Content Reach", value: pulse.contentReach.toLocaleString() },
    { label: "Client Outcomes", value: pulse.clientOutcomes }
  ];
    ...
    <div className="p-6 md:p-10 pb-32">
       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {pulseMetrics.map((p, idx) => (
           <div key={idx} className="p-4 bg-card border border-border rounded-[16px] shadow-sm flex flex-col gap-1 hover:bg-muted/30 transition-colors">
