...
export default function TeamPage() {
  ...
  const { team, work } = localData;  if (team.length === 0) {
    return (
  ...
  return (
    <div className="p-6 md:p-10 pb-32">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {team.map(member => {
          const activeTasksCount = work.filter(t => t.ownerId === member.id && (t.status === "IN_REVIEW")).length;
          const overdueTasksCount = work.filter(t => t.ownerId === member.id && new Date(t.dueDate).getTime() < Date.now() && t.status !== "COMPLETED").length;
          return (
               ...
               <div>
                 <div className="flex justify-between items-start mb-4">
                   ...
                   <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider leading-none ${
                     member.workload > member.capacity ? 'bg-destructive/20 text-destructive' :
                     member.workload === member.capacity ? 'bg-yellow-500/20 text-yellow-600' :
                   ...
                   <div className="grid grid-cols-2 gap-4 bg-muted/20 p-3 rounded-lg border border-border">
                     <div>
                       ...
                       <div className="flex items-end gap-2">
                          <span className={`text-[16px] font-bold leading-none ${member.workload > member.capacity ? 'text-destructive' : 'text-foreground'}`}>{member.workload}hrs</span> 
                       ...
                       <div className="w-full h-1 bg-secondary rounded-full mt-2 overflow-hidden">
                         <div className={`h-full ${member.workload > member.capacity ? 'bg-destructive' : 'bg-primary'}`} style={{ width: `${Math.min((member.workload/member.capacity)*100, 100)}%` }}></div>
                       </div>
                     </div>
                     <div>
