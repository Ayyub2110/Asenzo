...
export default function WorkPage() {
  ...
  if (error || !localData) return <div className="p-10">Error loading Work.</div>;

  const { work, team } = localData;

  if (!work || work.length === 0) {
    return (
      <div className="p-16 text-center text-muted-foreground bg-card rounded-[16px] border border-border mt-6 mx-6 md:mx-10">
        <h3 className="text-[14px] font-bold text-foreground mb-2">No operational work yet.</h3>
        <p className="text-[12px]">Create your first work item to begin tracking execution.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-10 pb-32">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest leading-none">Work Center</h2>
        <button className="px-4 py-2 bg-primary text-primary-foreground text-[12px] font-bold rounded-[6px]">Assign Work</button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[12px] whitespace-nowrap">
            <thead className="bg-muted/40 border-b border-border">
              <tr>
                <th className="px-6 py-4 font-bold text-muted-foreground uppercase tracking-wider">Work Item</th>
