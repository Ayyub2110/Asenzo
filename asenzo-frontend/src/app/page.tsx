  ...
  return (
    <div className="p-6 md:p-10 lg:p-12 max-w-[1360px] mx-auto w-full pb-32">
      ...
      <div className="flex flex-col mb-10">
         <h1 className="text-[24px] font-bold tracking-tight text-foreground uppercase mb-1">Command Center</h1>
      ...
      <section className="mb-10 grid grid-cols-1 lg:grid-cols-2 gap-6">
         ...
         {/* Business Foundation */}
         <div className="bg-card border border-border rounded-[16px] p-6 lg:p-8 hover:border-tertiary/40 transition-colors flex flex-col justify-between">
            <div>
               ...
               <div className="grid grid-cols-2 gap-4 text-[13px] mb-6">
                   <div>
                      <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">ICP</span>
                      <p className="font-medium text-foreground truncate">{foundation?.icp?.description || "Founder-led service businesses"}</p>
                   </div>
                   <div>
                      <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Core Pain</span>
                      <p className="font-medium text-foreground truncate">{foundation?.icp?.painPoints?.[0] || "Inconsistent growth"}</p>
                   </div>
                   <div>
                      <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Desired Result</span>
                      <p className="font-medium text-foreground truncate">{foundation?.icp?.desiredOutcomes?.[0] || "Predictable founder-led growth"}</p>
                   </div>
                   <div>
                      <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Positioning</span>
                      <p className="font-medium text-foreground truncate">{foundation?.coreDna?.positioning || "92 / 100"}</p>
                   </div>
                   <div className="col-span-2">
                       <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Unique Mechanism</span>
                       <p className="font-medium text-foreground">{foundation?.coreDna?.differentiation || "ASENZO Growth Operating System"}</p>
                   </div>
               </div>
            </div>
            <Link href="/foundation" className="flex items-center justify-center h-10 w-full bg-foreground text-background font-bold text-[13px] rounded-[8px] transition-opacity hover:opacity-90">
               View Foundation
            </Link>
         </div>
      </section>
      ...
      <section className="mb-10">
         ...
         <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
               { label: "Qualified Leads", value: intelligence?.pulse?.qualifiedLeads || cmd?.pulse[0].value, status: 'success' },
               { label: "Pipeline Value", value: `$${intelligence?.pulse?.pipeline?.toLocaleString() || '1.8M'}`, status: 'neutral' },
               { label: "Client Risk", value: intelligence?.pulse?.retentionRisk || 1, status: 'warning' },
               { label: "Content Queue", value: "8", status: 'attention' },
               { label: "Pending Approvals", value: cmd?.approvals.filter(a => a.status === 'PENDING_REVIEW').length || 3, status: 'destructive' }
            ].map((p, i) => (
                <div key={i} className="p-4 border border-border bg-card rounded-[12px] hover:border-tertiary/40 transition-colors">
         ...
         <section className="mb-10 relative">
           ...
           <div className="border border-destructive/30 bg-destructive/5 rounded-[12px] p-6 lg:p-8 flex flex-col md:flex-row gap-8 justify-between items-start">
             ...
             <div className="text-right shrink-0">
               ...
               <button className="bg-foreground text-background px-6 py-2.5 rounded-[8px] text-[13px] font-semibold transition-all hover:bg-foreground/90 shadow-sm" onClick={() => alert(`Routing to resolve constraint...`)}>
                 Resolve Constraint
               </button>
             </div>
           </div>
            ...
            <div className="flex flex-col gap-3">
               {cmd?.todayActions.slice(0, 3).map((action, i) => (
                  <div key={i} className="border border-border bg-card rounded-[12px] p-5 hover:border-tertiary/40 transition-colors flex items-center justify-between gap-4">
                     ...
                     <button className="shrink-0 bg-secondary text-foreground px-4 py-2 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
            ...
            <div className="flex flex-col gap-3">
               {cmd?.approvals.filter(a => a.status === 'PENDING_REVIEW').slice(0, 3).map((item, i) => (
                  <div key={i} className="flex items-center justify-between p-5 border border-border rounded-[12px] bg-card hover:border-tertiary/40 transition-colors">
                    ...
                    <button className="shrink-0 bg-foreground text-background px-4 py-2 rounded-[6px] text-[12px] font-bold hover:opacity-90 transition-opacity">
                       Review
                    </button>
                  </div>
               ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
         ...
         <section className="bg-card border border-border rounded-[12px] p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/delivery/retention" className="text-[11px] font-bold text-foreground uppercase tracking-widest hover:underline">View Library</Link>
            </div>
         ...
         <section className="bg-card border border-border rounded-[12px] p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/acquisition" className="text-[11px] font-bold text-foreground uppercase tracking-widest hover:underline">View Engine</Link>
            </div>
             ...
             {intelligence?.recommendations?.map(rec => (
                <div key={rec.id} className="bg-card border border-border rounded-[12px] p-5 flex flex-col md:flex-row gap-6 md:items-center justify-between">
                   ...
                   <div className="shrink-0 text-left md:text-right">
                      ...
                      <button className="bg-secondary text-foreground px-4 py-2 text-[12px] font-bold rounded-lg hover:bg-muted">Review Directive</button>
                   </div>
                </div>
             )) || (
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/intelligence" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Intelligence
            </Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/acquisition" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Acquisition
            </Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/conversion" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Conversion
            </Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/revenue" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Revenue
            </Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/delivery" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Delivery
            </Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-[12px] p-5 flex flex-col justify-between hover:border-tertiary/40 transition-colors">
            ...
            <Link href="/operations" className="block text-center border border-border w-full py-1.5 rounded-[6px] text-[12px] font-bold hover:bg-muted transition-colors">
              View Operations
            </Link>
          </div>
