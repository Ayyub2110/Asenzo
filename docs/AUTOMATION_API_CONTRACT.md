...
#### `POST /api/automation/audience-dna`
- **Purpose**: Persist canonical Audience DNA generated by the n8n C03 Audience DNA workflow. Maps structured psychological dimensions, buyer triggers, natural language snippets, and empirical evidence into `public.audience_dna`.
- **Headers**:
  - `x-automation-token: <secret>` (or `Authorization: Bearer <token>`)
  - `x-workspace-id: <workspace_id>` (optional if `workspace_id` is in body)
  - `Idempotency-Key: <uuid>` (optional for duplicate request prevention)
...
```json
{
  ...
  "audience_dna": {
    "core_pains": [
      "Founder calendar completely blocked by unqualified sales calls",
      "Stagnant growth despite high organic volume"
    ],
    "core_desires": [
      "Scale business without founder becoming the bottleneck",
      "Predictable automated client acquisition"
    ],
    "fears": [
      "Wasting months and budget on agencies that deliver zero qualified pipeline"
    ],
    "frustrations": [
      "Spending 15+ hours weekly on manual outreach"
    ],
    "beliefs": [
      "High-ticket B2B deals require founder-grade trust"
    ],
    "motivations": [
      "Transition from hands-on operator to true business owner"
    ],
    "aspirations": [
      "Build an automated $100k/mo acquisition engine"
    ],
    "objections": [
      "Our niche is too unique for automated systems"
    ],
    "decision_triggers": [
      "Hit capacity ceiling at 60 hours/week"
    ],
    "awareness_patterns": [
      "Problem-aware but unaware pipeline architecture is the bottleneck"
    ],
    "natural_language": [
      "I feel like I'm running on a treadmill that gets faster every month."
    ],
    "behaviors": [
      "Frequently publishes on LinkedIn without lead capture mechanisms"
    ],
    "relevant_audience_behaviors": [
      "Attends masterclasses and downloads templates without booking calls"
    ],
    "decision_criteria": [
      "Must integrate with existing calendar and CRM"
    ]
  },
  "evidence": [
    {
...
```json
{
  "success": true,
  "data": {
    "id": "adna_1726315200_a1b2",
    "workspace_id": "default-workspace",
    "situation": {
      "behaviors": ["Frequently publishes on LinkedIn without lead capture mechanisms"],
      "relevant_audience_behaviors": ["Attends masterclasses and downloads templates without booking calls"],
      "frustrations": ["Spending 15+ hours weekly on manual outreach"]
    },
    "pain": [
      "Founder calendar completely blocked by unqualified sales calls",
      "Stagnant growth despite high organic volume"
    ],
    "triggers": ["Hit capacity ceiling at 60 hours/week"],
    "objections": ["Our niche is too unique for automated systems"],
    "fears": ["Wasting months and budget on agencies that deliver zero qualified pipeline"],
    "natural_language": ["I feel like I'm running on a treadmill that gets faster every month."],
    "journey": {},
    "desired_future": {
      "core_desires": ["Scale business without founder becoming the bottleneck", "Predictable automated client acquisition"],
      "aspirations": ["Build an automated $100k/mo acquisition engine"],
      "motivations": ["Transition from hands-on operator to true business owner"]
    },
    "emotions": ["Transition from hands-on operator to true business owner"],
    "beliefs": ["High-ticket B2B deals require founder-grade trust"],
    "awareness": {
      "patterns": ["Problem-aware but unaware pipeline architecture is the bottleneck"]
    },
    "decision_criteria": ["Must integrate with existing calendar and CRM"],
    "buying_triggers": ["Hit capacity ceiling at 60 hours/week"],
    "evidence": [
      {
        "source_id": "interview_rec_42",
        "quote": "I'm working 60 hours a week and cannot take another sales call.",
...
- **Validation Rules**:
  ...
  - `audience_dna`: Required object.
  - Categories (`core_pains`, `core_desires`, `fears`, `objections`, etc.): All present categories must be arrays of strings.
  - `evidence`: Must be an array if provided.
  - `category` / `dna_type`: Must be `"AUDIENCE"` if provided.
- **Versioning Behavior**: Automatically determines the next sequential version number (`latest.version + 1`) per workspace. Each new POST creates an immutable historical version while activating the new version for downstream context queries (`fetchAudienceContext`).
- **Status Codes**:
  - `201 Created`: Successfully persisted canonical Audience DNA.
  - `400 Bad Request`: Missing `workspace_id`, missing `audience_dna`, or schema validation failure.
  - `401 Unauthorized`: Missing or invalid `x-automation-token`.
  - `500 Internal Server Error`: Database insertion or connection failure surfaced explicitly.

#### `GET /api/automation/audience-dna`
- **Purpose**: Retrieve the latest canonical Audience DNA record for a workspace.
- **Query Params**: `?workspace_id=default-workspace`
- **Headers**: `x-automation-token: <secret>`
- **Response (200 OK)**:
```json
{
